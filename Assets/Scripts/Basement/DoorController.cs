...
public class DoorController : MonoBehaviour
{
    ...
    void Update()
    {
        if (Vector3.Distance(character.transform.position, transform.position) <= 1f)
        {
            ...
            } else if (doorCon.name.Equals("OpeningDoor") && ControllerCharacter.coreItems < 9) {
                StartCoroutine(pm.setAlertText("Core Items < 9"));
            }
            ...
            { 
                ...
                victoryFlag = true;
                StartCoroutine(pm.setAlertText("Press F to ride"));
            }
